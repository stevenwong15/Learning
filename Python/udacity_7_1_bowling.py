...
# instead of keeping track indicies of the balls
# use balls as a queue: pop and look at up to the first 3 scores
def bowling(balls):
    "Compute the total score for a player's game of bowling."
    score = list(range(10))
    for i in range(10):
        score[i], balls = score_frame(balls)
    return sum(score)

def score_frame(balls):
    if balls[0] == 10:
        return 10 + balls[1] + balls[2], balls[1:]
    elif balls[0] + balls[1] == 10:
        return 10 + balls[2], balls[2:]
    else:
